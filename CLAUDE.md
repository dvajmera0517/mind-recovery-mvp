...
- `src/mind_recovery_mvp/evidence_excerpts.py` — short, paraphrased, non-verbatim source material for statins/diuretics/ppi (data only, not prose in code), used exclusively as `scripts/draft_content.py`'s input. Not a clinical database and not citation-ready — `citation_label` is carried forward verbatim as `clinical_source`, never turned into a more specific invented citation.
