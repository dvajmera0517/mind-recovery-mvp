...
def is_customer_visible(content_status: str) -> bool:
    ...
    original PLACEHOLDER state and the LLM-drafted-pending state are
    explicitly hidden, even if some of their fields happen to be
    populated (e.g. diuretics' PLACEHOLDER record already has a non-null
    foods_that_may_help) — drafting or partial seeding doesn't change
    what's safe to show a customer.
    ...
    LLM-draft/review pipeline entirely — review_content.py never touches
    it either, since it only lists STATUS_LLM_DRAFTED_PENDING_REVIEW
    records. This gate protects the new pipeline; it isn't a blanket
    "only formally approved content is ever shown" rule.
    """
    if content_status in APPROVED_STATUSES:
        return True
    if content_status == STATUS_LLM_DRAFTED_PENDING_REVIEW:
        return False
